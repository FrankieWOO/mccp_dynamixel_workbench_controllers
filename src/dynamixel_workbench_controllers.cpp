...
bool DynamixelController::initGroupBulkWrite(void)
{
  bool result = false;
  const char* log = NULL;
  result = dxl_wb_->initBulkWrite(&log);
  if (result == false)
  {
    ROS_ERROR("%s", log);
    return result;
...
void DynamixelController::initSubscriber()
{
  ...
  dxlcmd2_sub_ = priv_node_handle_.subscribe("dxlcmd2", 10, &DynamixelController::dxlcmd2MsgCallback, this);
...
void DynamixelController::initServer()
{
  ...
  dynamixel_command_server2_ = priv_node_handle_.advertiseService("dynamixel_command2", &DynamixelController::dynamixelCommandTwoMsgCallback, this);
}
...
// created by F. Wu
void DynamixelController::dxlcmd2MsgCallback(const dynamixel_workbench_msgs::Dxlcmd2 &msg)
{
  bool result = false;
  const char* log;
  ...
  int32_t value1 = msg.value1;
  int32_t value2 = msg.value2;

  result = dxl_wb_->addBulkWriteParam(id1, item_name.c_str(), value1, &log);

  if (result == false)
  {
    ROS_ERROR("%s", log);
    ROS_ERROR("Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", value1, item_name.c_str(), id1);
  }
  
  result = dxl_wb_->addBulkWriteParam(id2, item_name.c_str(), value2, &log);
  if (result == false)
  {
    ROS_ERROR("%s", log);
    ROS_ERROR("Failed to write value[%d] on items[%s] to Dynamixel[ID : %d]", value2, item_name.c_str(), id2);
  }
  result = dxl_wb_->bulkWrite(&log);
  if (result == false)
  {
    ROS_ERROR("%s", log);
    ROS_ERROR("Failed to bulk write");
  }
}
...
bool DynamixelController::dynamixelCommandTwoMsgCallback(dynamixel_workbench_msgs::DynamixelCommandTwo::Request &req,
                                                      dynamixel_workbench_msgs::DynamixelCommandTwo::Response &res)
{
  bool result = false;
  const char* log;
  ...
  uint8_t id2 = req.id2;
  std::string item_name = req.addr_name;
  int32_t value1 = req.value1;
  int32_t value2 = req.value2;
...
int main(int argc, char **argv)
{
  ...
  result = dynamixel_controller.initGroupBulkWrite();
